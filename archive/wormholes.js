...
readReverseLines(function(lines) {
  ...
  return writeOutputs(yn);
});

readUniverse = function(lines) {
  var graph, numGalaxies, numWormholes, wormholes, _ref;
  _ref = nextLine(toArray(toInt), lines), numGalaxies = _ref[0], numWormholes = _ref[1];
...
canGoBackForever = function(graph) {
  var goesBack;
  goesBack = function(vertex, dt, visited) {
    var canBack, edge, _i, _len, _ref;
    if (visited == null) {
      visited = {};
    }
    if (visited[vertex.key] != null) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      edge = _ref[_i];
      canBack = goesBack(graph.vertex(edge.to), dt + edge.weight, Object.create(visited));
      if (canBack) {
        return true;
      }
    }
