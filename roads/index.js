...
var Edge, Graph, Infinity, Vertex, bellmanFord, curry, graphToDijkstraCompatible, lines, mapn, nextLine, nextLines, notEmpty, parseGraphEdge, readLine, readLines, readObjects, readReverseLines, readStdin, toArray, toGraph, toInt, toInts, toObject, toString, words, writeOutputs, _,
  __slice = [].slice;
...
// Generated by CoffeeScript 1.3.3
var city, clone, countryDistance, min, readCountries, readCountry, shortestDistance, toCountry, toRoad;

min = _.min;

clone = _.clone;

countryDistance = function(country) {
  var distance;
  return distance = shortestDistance(country.cities, country.dest, country.coins);
};

readCountries = function(lines) {
  var countries, numCountries;
  numCountries = readLine(toInt, lines);
  return countries = (function() {
    var _i, _results;
    _results = [];
    for (_i = 0; 0 <= numCountries ? _i < numCountries : _i > numCountries; 0 <= numCountries ? _i++ : _i--) {
      _results.push(readCountry(lines));
    }
    return _results;
  })();
};

readCountry = function(lines) {
  var coins, numCities, numRoads, roads;
  ...
  return toCountry(coins, numCities, roads);
};

toRoad = function(line) {
  var from, to, toll, weight, _ref;
  _ref = words(line).map(toInt), from = _ref[0], to = _ref[1], weight = _ref[2], toll = _ref[3];
  return {
    from: from,
    to: to,
    weight: weight,
...
toCountry = function(coins, numCities, roads) {
  return {
    coins: coins,
    dest: numCities,
    cities: toGraph(roads)
  };
};

city = function(cities, id) {
  var _ref;
  if ((_ref = cities[id]) == null) {
    cities[id] = [];
  }
  return cities[id];
};
...
shortestDistance = function(graph, destId, coins) {
  var walk;
  walk = function(vertex, coins, visited) {
    var distance, distances, uv, v, valid;
    if (visited == null) {
      visited = {};
    }
    ...
    distances = (function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        uv = _ref[_i];
        v = graph.vertex(uv.to);
        distance = walk(v, coins - uv.toll, visited);
        if (distance >= 0) {
          _results.push(distance + uv.weight);
        } else {
          _results.push(-1);
        }
      }
    ...
    valid = distances.filter(function(d) {
      return d >= 0 && d < Infinity;
    });
    return min(valid);
  };
  return walk(graph.vertex(1), coins);
};
