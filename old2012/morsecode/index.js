...
var allCodes, code, letter, map, max, maxCodeLength, reverseMap;
...
for (letter in reverseMap) {
  code = reverseMap[letter];
...
readReverseLines(function(lines) {
  var joined, possibles;
  code = readString(lines);
  possibles = allCodes(code);
  joined = possibles.map(function(p) {
    return p.join('');
  });
  return writeOutputs(joined.sort());
});

allCodes = function(code) {
  var four, matches, one, subcode, subcodes, three, two, _i, _j, _k, _l, _len, _len1, _len2, _len3;
  one = map[code.substr(0, 1)];
  two = map[code.substr(0, 2)];
  three = map[code.substr(0, 3)];
  four = map[code.substr(0, 4)];
  matches = [];
  if (code.length > 0 && one) {
    subcodes = allCodes(code.substr(1));
    if (!subcodes.length) {
      matches.push(one);
    } else {
      for (_i = 0, _len = subcodes.length; _i < _len; _i++) {
        subcode = subcodes[_i];
        matches.push([one].concat(subcode));
      }
    }
  }
  if (code.length > 1 && two) {
    subcodes = allCodes(code.substr(2));
    if (!subcodes.length) {
      matches.push(two);
    } else {
      for (_j = 0, _len1 = subcodes.length; _j < _len1; _j++) {
        subcode = subcodes[_j];
        matches.push([two].concat(subcode));
      }
    }
  }
  if (code.length > 2 && three) {
    subcodes = allCodes(code.substr(3));
    if (!subcodes.length) {
      matches.push(three);
    } else {
      for (_k = 0, _len2 = subcodes.length; _k < _len2; _k++) {
        subcode = subcodes[_k];
        matches.push([three].concat(subcode));
      }
    }
  }
  if (code.length > 3 && four) {
    subcodes = allCodes(code.substr(4));
    if (!subcodes.length) {
      matches.push(four);
    } else {
      for (_l = 0, _len3 = subcodes.length; _l < _len3; _l++) {
        subcode = subcodes[_l];
