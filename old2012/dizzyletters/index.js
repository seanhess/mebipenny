...
var max, padSpace, rotate, rotateOnce, toArray, toLineString;

max = _.max;

readReverseLines(function(lines) {
  var arrays, degrees, ls, maxstring, padded, rotated, string, strings;
  degrees = readInts(lines)[0];
  strings = [];
  while ((string = readString(lines))) {
    strings.push(string);
  }
  maxstring = max(strings, function(s) {
    return s.length;
  });
...
padSpace = curry(function(max, string) {
...
rotate = function(degrees, arrays) {
  var clockwiseRotations;
...
rotateOnce = function(arrays) {
  var dest, ox, oy, row, x, y, _i, _j, _ref, _ref1, _ref2;
  dest = [];
  max = arrays.length - 1;
  for (y = _i = 0, _ref = arrays[0].length; 0 <= _ref ? _i < _ref : _i > _ref; y = 0 <= _ref ? ++_i : --_i) {
    row = arrays[y];
    for (x = _j = 0, _ref1 = arrays.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; x = 0 <= _ref1 ? ++_j : --_j) {
      ox = y;
      oy = max - x;
      if ((_ref2 = dest[y]) == null) {
        dest[y] = [];
      }
